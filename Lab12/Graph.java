...
public class Graph {
    ...
    public Queue<Pair<Integer, Float>> MSTqueue;
    public HashMap<Integer, GraphNode> verts;
    public HashMap<Integer, List<Pair<Integer, Float>>> edges;
    public int numNodes;
    public int endKey;
    public String name;
    private int time;
    ...
    public Graph(String path, boolean dir, boolean weight) throws IOException {
        ...
        try { 
            ...
            if(!this.weighted) {
                ...
                for (String line2 : lines) {
                    String[] edge = line2.split(" ");
                    addEdge(Integer.parseInt(edge[0]), Integer.parseInt(edge[1]), Float.parseFloat(edge[3]));
            }
        }   
        finally {
            if(in != null){
    ...
    public void addEdge(int key1, int key2, float weight) {
        ...
        List wList = edges.getOrDefault(key1, new ArrayList<Pair<Integer, Float>>());
    ...
    public void MSTPrim(int key) {
        ...
        Collection<GrapNode> vertsC = verts.values();
        while(!vertsC.isEmpty()) {
            Graphnode u = extractMin(vertsC);
            for(GraphNode v : u.neighbors) {
                if(vertsC.contains(v) && ) {
    ...
    private void extractMin(Collection<GraphNode> v) {
